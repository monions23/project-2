...
function getBudgetTooltipLine(countyName, mode) {
  ...
  if (m === "total")
    return "Total expenditures: <strong>" + fmt(d.total) + "</strong>";
  if (m === "safety")
    return "Public safety: <strong>" + fmt(d.safety) + "</strong>";
  if (m === "health")
    return "Health & social: <strong>" + fmt(d.health) + "</strong>";
  if (m === "roads")
    return "Roads & transport: <strong>" + fmt(d.roads) + "</strong>";
  if (m === "admin")
    return "Administration: <strong>" + fmt(d.admin) + "</strong>";
  if (m === "education")
    return "Environment & education: <strong>" + fmt(d.education) + "</strong>";
  return "No data";
}
...
function setBudgetMode(mode) {
  ...
  registerLayer(
    "budget:" + mode,
    _budgetLayerLabel(mode),
    _budgetLegendColors(mode),
  );
  repaintWithBlend();
}
...
function refreshBudgetLayer() {
  ...
  var checked = document.querySelectorAll(
    'input[name="budget-metric"]:checked',
  );
  ...
  checked.forEach(function (cb) {
    registerLayer(
      "budget:" + cb.value,
      _budgetLayerLabel(cb.value),
      _budgetLegendColors(cb.value),
    );
  });
  repaintWithBlend();
}
...
function buildBudgetData(csvData) {
  ...
  csvData.forEach(function (row) {
    ...
    county = county.toLowerCase().replace(/\b\w/g, function (c) {
