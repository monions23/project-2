...
function attachLiquorHoverEvents() {
  ...
  MapEvents.addEventListener("map-mouseover", (e) => {
    getActiveCounties();
    console.log("mouseover");
    var currName = e.detail.name;
    console.log(currName);

    if (!liquorData[currName]) return;
    ...
    liquorTooltip
      ...
      .html(
        "<strong>" +
          currName +
          " County</strong><br>" +
          getLiquorTooltipLine(currName),
      );
  });

  MapEvents.addEventListener("map-mousemove", (e) => {
    if (!liquorTooltip) return;

    const { event } = e.detail;
    const [x, y] = d3.pointer(event, d3.select("#iowa-map").node());

    liquorTooltip.style("left", x + 14 + "px").style("top", y - 28 + "px");
  });

  MapEvents.addEventListener("map-mouseout", () => {
